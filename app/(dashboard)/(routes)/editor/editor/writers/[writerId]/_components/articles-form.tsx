...
export const ArticlesForm = ({ initialData, writerId }: ArticlesFormProps) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      await axios.post(`/api/writers/${writerId}/articles`, values);
    ...
    <div className="relative mt-6 border bg-slate-100 rounded-md p-4">
                ...
                <FormItem>
                  <FormControl>
                    <Input
                      ...
                      placeholder="e.g., 'New technology for Africa'"
                      {...field}
                    />
                  </FormControl>
                  <FormMessage />
      ...
      {!isCreating && (
        <div
          className={cn(
            "text-sm mt-2",
            !initialData.articles.length && "text-slate-500 italic"
          )}
        >
          {!initialData.articles.length && "No articles"}
          <ArticlesList
            onEdit={onEdit}
            onReorder={onReorder}
            items={initialData.articles || []}
          />
        </div>
      )}
      {!isCreating && (
        <p className="text-xs text-muted-foreground mt-4">
          Drag and drop to reorder the articles
        </p>
      )}
    </div>
  );
